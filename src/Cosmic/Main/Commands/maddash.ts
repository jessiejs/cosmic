...
export async function maddash_run(
	room: Room,
	_message: Message,
...
You have 30 seconds!`);
		...
		setTimeout(() => {
			...
			if (game.smallestAuthor) {
				room.send(`🌟 The winner is **${
					getUserData(game.smallestAuthor)?.username
				}**!
They wrote '${game.smallest}', which is only ${
					game.smallest.length
				} characters long!
