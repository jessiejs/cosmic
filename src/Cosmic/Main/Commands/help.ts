...
export function help_run(room: Room, _message:Message, _context: ParseContext) {
	const text = `**🔮 Cosmic Help**

${[...helpCommands].map(c => `- ${c[1].emoji}  **${prefix}${c[0]}**: ${c[1].description}
	Syntax: ${prefix}${c[1].syntax}`).join('\n\n')}`;
...
export const helpCommands: Map<string,{
	emoji:string,
	name:string,
	description:string,
	syntax:string
	...
	['color', {
		emoji: '🎨',
		name: 'color',
		description: 'Shows the color of a user, or your color if none is specified.',
	...
	['id', {
		emoji: '🆔',
		name: 'id',
		description: 'Shows the id of a user, or your id if none is specified.',
		syntax: 'id <user (optional)>'
	}],
	['info', {
		emoji: 'i️',
		name: 'info',
		description: 'Shows the info of a user, or your info if none is specified.',
