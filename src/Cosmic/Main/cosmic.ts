...
import { premium_run } from './Commands/premium.ts';
...
export function cosmic(room: Room) {
	...
	room.addEventListener('message', async message => {
		...
		if (message.text.toLowerCase().includes('@cosmic')) {
			const worker = new Worker(
				new URL('../AI/aiThread.worker.ts', import.meta.url),
				{
					type: 'module',
				}
			);
		...
		if (
			getUserID(message) === room.getCosmicId() &&
			message.username.toLowerCase().includes('cosmic')
		) {
			return;
		}
 ...
 | The message above is from a fake!`);
		...
		const lastMessageFromUserDay =
			((await db.get(['lastMessageFromUserDay'])).value as number) || 0;
...
Next message will be ${count + 2}`);
		...
		if (madDash.game) {
			...
			if (isDash) {
				...
				if (message.text.length < madDash.game.smallest.length) {
					...
					room.send(
						`${message.text.length} characters... **but the record is ${madDash.game.smallest.length}!**`
					);
				}
			}
		}
		...
		if (
			latestUserMessages[message.userID || message.username] &&
			latestUserMessages[message.userID || message.username].username !==
				message.username
		) {
			room.send(
				`${
